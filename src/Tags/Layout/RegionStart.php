...
use Lsa\Xsl\Core\Validation\Tags\RegionStart as ValidateRegionStart;
use Lsa\Xsl\Core\Xml\Tag;

/**
 * Used in constructing a simple-page-master. This region specifies a viewport/reference pair that is located
 * on the "start" side of the page-reference-area. In lr-tb writing-mode, this region corresponds to a left
 * sidebar. The overflow trait controls how much of the underlying region-reference-area is visible; that is,
 * whether the region-reference-area is clipped by its parent region-viewport-area.
 *
 * Areas:
 * The fo:region-start formatting object is used to generate one region-viewport-area and one
 * region-reference-area.
 * The values of the padding and border-width traits must be "0".
 * The start-edge of the content-rectangle of this region-viewport-area is positioned coincident with the
 * start-edge of the content-rectangle of the page-reference-area generated using the parent
 * fo:simple-page-master. The inline-progression-dimension of the region-viewport-area is determined by the
 * extent trait on the fo:region-start formatting object.
 * The block-progression-dimension of the region-viewport-area is determined by the precedence trait on the
 * adjacent fo:region-before and the fo:region-after, if these exist; otherwise it is determined as if the
 * value of the precedence trait was false. If the value of the precedence trait of the fo:region-before (or,
 * respectively, fo:region-after) is false, then the block-progression-dimension extends up to the before-
 * (or, respectively, after-) edge of the content-rectangle of the page-reference-area. In this case, the
 * region-start acts like a float into areas generated by the region-before (respectively, the region-after).
 * If the value of the precedence trait on the adjacent regions is true, then these adjacent regions float
 * into the area generated by the fo:region-start and the extent of the fo:region-start is (effectively)
 * reduced by the incursions of the adjacent regions with the value of the precedence trait equal to true.
 ...
 *
 * Constraints:
 * The constraints on the size and position of the region-reference-area generated using the fo:region-start
 ...
 * Contents:
 * EMPTY
 * @see https://www.w3.org/TR/xsl11/#fo_region-start
 */
class RegionStart extends Tag implements HasAuralFallback, HasVisualFallback
{
    use ExtendedAuralConformance;
    use ExtendedVisualConformance;
    use ValidateRegionStart;
