...
/**
 * Validates FlowNameReference property.
 *
 * `<name>`:
 *     The name identifies a flow; it may not be empty and must refer to a flow-name that exists within the
 *     document.
 *
 * @link https://www.w3.org/TR/xsl11/#flow-name-reference
 */
class FlowNameReferenceType extends Type implements DataAwareValidator
{
    ...
    public function validateWithContext(string $value, ?Tag $root, ?EmptyTag $current): bool
    {
        ...
        if ($root === null) {
            throw new ValidationException('FlowNameReferenceType needs root context to validate value.');
        }
