...
/**
 * Validates MasterReference property.
 *
 * @link https://www.w3.org/TR/xsl11/#master-reference
 */
class MasterReferenceType extends Type implements DataAwareValidator
{
    ...
    {
        // A master-reference must comply with NCName
        ...
        if ($root === null) {
            throw new ValidationException('MasterReferenceType needs root context to validate value.');
        }

        // A master-reference must match an ID property
        $tagsWithIds = $root->xpath('//*[@master-name="'.$value.'"]');
