...
final class TextTransformType extends CommonMethodsFop
{
    public function testSchema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT | XmlComparator::VALIDATION_SWAPPABLE_ATTRIBUTE_VALUES);
        $xsd = self::$xsdElement->xpath('/x:schema/x:simpleType[@name="text_transform_Type"]');
        $fop = self::$fopElement->xpath('/x:schema/x:simpleType[@name="text_transform_Type"]');
