...
final class SpaceBeforeOptimumTypeTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT | XmlComparator::VALIDATION_SWAPPABLE_ATTRIBUTE_VALUES);
        $xsd = self::$xsdElement->xpath('/x:schema/x:simpleType[@name="space_before_optimum_Type"]');
        $fop = self::$fopElement->xpath('/x:schema/x:simpleType[@name="space_before_optimum_Type"]');
