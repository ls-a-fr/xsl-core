...
final class StaticContentTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="static-content"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="static-content"]');
    ...
    {
        $tag1 = (new StaticContent())->asDefinition();
        $tag2 = (new FopProfile())->check(new StaticContent())->asDefinition();
