...
final class TableFooterTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="table-footer"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="table-footer"]');
    ...
    {
        $tag1 = (new TableFooter())->asDefinition();
        $tag2 = (new FopProfile())->check(new TableFooter())->asDefinition();
