...
final class RepeatablePageMasterAlternativesTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="repeatable-page-master-alternatives"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="repeatable-page-master-alternatives"]');
    ...
    {
        $tag1 = (new RepeatablePageMasterAlternatives())->asDefinition();
        $tag2 = (new FopProfile())->check(new RepeatablePageMasterAlternatives())->asDefinition();
