...
final class RegionStartTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="region-start"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="region-start"]');
    ...
    {
        $tag1 = (new RegionStart())->asDefinition();
        $tag2 = (new FopProfile())->check(new RegionStart())->asDefinition();
