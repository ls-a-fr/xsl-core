...
final class ListBlockTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="list-block"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="list-block"]');
    ...
    {
        $tag1 = (new ListBlock())->asDefinition();
        $tag2 = (new FopProfile())->check(new ListBlock())->asDefinition();
