...
final class BlockContainerTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="block-container"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="block-container"]');
    ...
    {
        $tag1 = (new BlockContainer())->asDefinition();
        $tag2 = (new FopProfile())->check(new BlockContainer())->asDefinition();
