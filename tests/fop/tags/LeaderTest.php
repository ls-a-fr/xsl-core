...
final class LeaderTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="leader"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="leader"]');
    ...
    {
        $tag1 = (new Leader())->asDefinition();
        $tag2 = (new FopProfile())->check(new Leader())->asDefinition();
