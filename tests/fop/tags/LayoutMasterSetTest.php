...
final class LayoutMasterSetTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="layout-master-set"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="layout-master-set"]');
    ...
    {
        $tag1 = (new LayoutMasterSet())->asDefinition();
        $tag2 = (new FopProfile())->check(new LayoutMasterSet())->asDefinition();
