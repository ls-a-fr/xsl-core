...
final class TitleTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="title"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="title"]');
    ...
    {
        $tag1 = (new Title())->asDefinition();
        $tag2 = (new FopProfile())->check(new Title())->asDefinition();
