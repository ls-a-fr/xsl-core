...
final class WrapperTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="wrapper"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="wrapper"]');
    ...
    {
        $tag1 = (new Wrapper())->asDefinition();
        $tag2 = (new FopProfile())->check(new Wrapper())->asDefinition();
