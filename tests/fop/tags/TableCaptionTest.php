...
final class TableCaptionTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="table-caption"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="table-caption"]');
    ...
    {
        $tag1 = (new TableCaption())->asDefinition();
        $tag2 = (new FopProfile())->check(new TableCaption())->asDefinition();
