...
final class PageSequenceMasterTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="page-sequence-master"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="page-sequence-master"]');
    ...
    {
        $tag1 = (new PageSequenceMaster())->asDefinition();
        $tag2 = (new FopProfile())->check(new PageSequenceMaster())->asDefinition();
