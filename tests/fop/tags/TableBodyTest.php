...
final class TableBodyTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="table-body"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="table-body"]');
    ...
    {
        $tag1 = (new TableBody())->asDefinition();
        $tag2 = (new FopProfile())->check(new TableBody())->asDefinition();
