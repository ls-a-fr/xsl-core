...
final class RetrieveMarkerTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="retrieve-marker"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="retrieve-marker"]');
    ...
    {
        $tag1 = (new RetrieveMarker())->asDefinition();
        $tag2 = (new FopProfile())->check(new RetrieveMarker())->asDefinition();
