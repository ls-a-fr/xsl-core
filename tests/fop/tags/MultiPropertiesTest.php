...
final class MultiPropertiesTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="multi-properties"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="multi-properties"]');
    ...
    {
        $tag1 = (new MultiProperties())->asDefinition();
        $tag2 = (new FopProfile())->check(new MultiProperties())->asDefinition();
