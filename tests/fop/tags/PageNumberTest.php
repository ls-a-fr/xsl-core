...
final class PageNumberTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="page-number"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="page-number"]');
    ...
    {
        $tag1 = (new PageNumber())->asDefinition();
        $tag2 = (new FopProfile())->check(new PageNumber())->asDefinition();
