...
final class RegionBodyTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="region-body"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="region-body"]');
    ...
    {
        $tag1 = (new RegionBody())->asDefinition();
        $tag2 = (new FopProfile())->check(new RegionBody())->asDefinition();
