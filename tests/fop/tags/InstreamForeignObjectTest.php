...
final class InstreamForeignObjectTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="instream-foreign-object"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="instream-foreign-object"]');
    ...
    {
        $tag1 = (new InstreamForeignObject())->asDefinition();
        $tag2 = (new FopProfile())->check(new InstreamForeignObject())->asDefinition();
