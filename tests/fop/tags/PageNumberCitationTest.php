...
final class PageNumberCitationTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="page-number-citation"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="page-number-citation"]');
    ...
    {
        $tag1 = (new PageNumberCitation())->asDefinition();
        $tag2 = (new FopProfile())->check(new PageNumberCitation())->asDefinition();
