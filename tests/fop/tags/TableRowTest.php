...
final class TableRowTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="table-row"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="table-row"]');
    ...
    {
        $tag1 = (new TableRow())->asDefinition();
        $tag2 = (new FopProfile())->check(new TableRow())->asDefinition();
