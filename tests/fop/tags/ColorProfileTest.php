...
final class ColorProfileTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="color-profile"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="color-profile"]');
    ...
    {
        $tag1 = (new ColorProfile())->asDefinition();
        $tag2 = (new FopProfile())->check(new ColorProfile())->asDefinition();
