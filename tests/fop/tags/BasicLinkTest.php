...
final class BasicLinkTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="basic-link"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="basic-link"]');
    ...
    {
        $tag1 = (new BasicLink())->asDefinition();
        $tag2 = (new FopProfile())->check(new BasicLink())->asDefinition();
