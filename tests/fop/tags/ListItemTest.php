...
final class ListItemTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="list-item"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="list-item"]');
    ...
    {
        $tag1 = (new ListItem())->asDefinition();
        $tag2 = (new FopProfile())->check(new ListItem())->asDefinition();
