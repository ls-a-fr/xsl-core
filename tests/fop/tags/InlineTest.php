...
final class InlineTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="inline"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="inline"]');
    ...
    {
        $tag1 = (new Inline())->asDefinition();
        $tag2 = (new FopProfile())->check(new Inline())->asDefinition();
