...
final class RootTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="root"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="root"]');
    ...
    {
        $tag1 = (new Root())->asDefinition();
        $tag2 = (new FopProfile())->check(new Root())->asDefinition();
