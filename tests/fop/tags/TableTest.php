...
final class TableTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="table"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="table"]');
    ...
    {
        $tag1 = (new Table())->asDefinition();
        $tag2 = (new FopProfile())->check(new Table())->asDefinition();
