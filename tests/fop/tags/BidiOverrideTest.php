...
final class BidiOverrideTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="bidi-override"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="bidi-override"]');
    ...
    {
        $tag1 = (new BidiOverride())->asDefinition();
        $tag2 = (new FopProfile())->check(new BidiOverride())->asDefinition();
