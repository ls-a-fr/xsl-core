...
final class FootnoteBodyTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="footnote-body"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="footnote-body"]');
    ...
    {
        $tag1 = (new FootnoteBody())->asDefinition();
        $tag2 = (new FopProfile())->check(new FootnoteBody())->asDefinition();
