...
final class MultiToggleTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="multi-toggle"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="multi-toggle"]');
    ...
    {
        $tag1 = (new MultiToggle())->asDefinition();
        $tag2 = (new FopProfile())->check(new MultiToggle())->asDefinition();
