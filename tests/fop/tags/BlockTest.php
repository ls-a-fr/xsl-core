...
final class BlockTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="block"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="block"]');
    ...
    {
        $tag1 = (new Block())->asDefinition();
        $tag2 = (new FopProfile())->check(new Block())->asDefinition();
