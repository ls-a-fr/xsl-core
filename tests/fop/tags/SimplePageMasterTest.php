...
final class SimplePageMasterTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="simple-page-master"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="simple-page-master"]');
    ...
    {
        $tag1 = (new SimplePageMaster())->asDefinition();
        $tag2 = (new FopProfile())->check(new SimplePageMaster())->asDefinition();
