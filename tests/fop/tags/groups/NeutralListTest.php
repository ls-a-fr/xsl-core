...
final class NeutralListTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:group[@name="neutral_List"]');
        $fop = self::$fopElement->xpath('/x:schema/x:group[@name="neutral_List"]');
