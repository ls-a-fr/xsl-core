...
final class FloatListTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:group[@name="float_List"]');
        $fop = self::$fopElement->xpath('/x:schema/x:group[@name="float_List"]');
