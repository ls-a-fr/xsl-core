...
final class FootnoteListTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:group[@name="footnote_List"]');
        $fop = self::$fopElement->xpath('/x:schema/x:group[@name="footnote_List"]');
