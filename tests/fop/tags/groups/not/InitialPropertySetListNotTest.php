...
namespace Lsa\Xsl\Core\Tests\Fop\Tags\Groups\Not;

use Lsa\Xml\Utils\Xml\XmlComparator;
use Lsa\Xsl\Core\Tests\Fop\CommonMethodsFop;

final class InitialPropertySetListNotTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:group[@name="initial_property_set_List_Not"]');
        $fop = self::$fopElement->xpath('/x:schema/x:group[@name="initial_property_set_List_Not"]');
