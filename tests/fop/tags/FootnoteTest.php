...
final class FootnoteTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="footnote"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="footnote"]');
    ...
    {
        $tag1 = (new Footnote())->asDefinition();
        $tag2 = (new FopProfile())->check(new Footnote())->asDefinition();
