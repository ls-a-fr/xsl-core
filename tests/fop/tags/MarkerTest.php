...
final class MarkerTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="marker"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="marker"]');
    ...
    {
        $tag1 = (new Marker())->asDefinition();
        $tag2 = (new FopProfile())->check(new Marker())->asDefinition();
