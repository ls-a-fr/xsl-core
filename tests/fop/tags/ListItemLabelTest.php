...
final class ListItemLabelTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="list-item-label"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="list-item-label"]');
    ...
    {
        $tag1 = (new ListItemLabel())->asDefinition();
        $tag2 = (new FopProfile())->check(new ListItemLabel())->asDefinition();
