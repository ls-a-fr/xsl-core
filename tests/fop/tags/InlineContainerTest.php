...
final class InlineContainerTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="inline-container"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="inline-container"]');
    ...
    {
        $tag1 = (new InlineContainer())->asDefinition();
        $tag2 = (new FopProfile())->check(new InlineContainer())->asDefinition();
