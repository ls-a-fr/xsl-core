...
final class FloatElementTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="float"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="float"]');
    ...
    {
        $tag1 = (new FloatElement())->asDefinition();
        $tag2 = (new FopProfile())->check(new FloatElement())->asDefinition();
