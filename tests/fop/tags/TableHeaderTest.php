...
final class TableHeaderTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="table-header"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="table-header"]');
    ...
    {
        $tag1 = (new TableHeader())->asDefinition();
        $tag2 = (new FopProfile())->check(new TableHeader())->asDefinition();
