...
final class ConditionalPageMasterReferenceTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="conditional-page-master-reference"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="conditional-page-master-reference"]');
    ...
    {
        $tag1 = (new ConditionalPageMasterReference())->asDefinition();
        $tag2 = (new FopProfile())->check(new ConditionalPageMasterReference())->asDefinition();
