...
final class RegionBeforeTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:element[@name="region-before"]');
        $fop = self::$fopElement->xpath('/x:schema/x:element[@name="region-before"]');
    ...
    {
        $tag1 = (new RegionBefore())->asDefinition();
        $tag2 = (new FopProfile())->check(new RegionBefore())->asDefinition();
