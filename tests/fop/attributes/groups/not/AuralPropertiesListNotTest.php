...
final class AuralPropertiesListNotTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:attributeGroup[@name="Aural_Properties_List_Not"]');
        $fop = self::$fopElement->xpath('/x:schema/x:attributeGroup[@name="Aural_Properties_List_Not"]');
