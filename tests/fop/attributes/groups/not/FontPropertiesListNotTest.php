...
final class FontPropertiesListNotTest extends CommonMethodsFop
{
    public function test_schema(): void
    {
        $comparator = new XmlComparator(XmlComparator::VALIDATION_STRICT);
        $xsd = self::$xsdElement->xpath('/x:schema/x:attributeGroup[@name="Font_Properties_List_Not"]');
        $fop = self::$fopElement->xpath('/x:schema/x:attributeGroup[@name="Font_Properties_List_Not"]');
