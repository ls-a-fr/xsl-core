...
use Lsa\Xsl\Core\Tests\Providers\RgbFunctionDataProvider;
use Lsa\Xsl\Core\Tokenizer\Computing\XslTokenParser;
use Lsa\Xsl\Core\Validation\Types\Functions\ColorRgbType;
use PHPUnit\Framework\Attributes\DataProviderExternal;

final class RgbFunctionTest extends CommonMethods
{
    #[DataProviderExternal(RgbFunctionDataProvider::class, 'validDataProvider')]
    public function test_validation_with_valid_values(string $value, string $expected): void
    {
        XslTokenParser::make();
        [$root, $attribute] = $this->mockRoot();
        $this->assertEquals(XslTokenParser::parse($value, $root, $attribute)->evaluate(), $expected);
    }

    #[DataProviderExternal(RgbFunctionDataProvider::class, 'invalidDataProvider')]
    public function test_validation_with_invalid_values(string $value, string $expected, ?string $exceptionClass): void
    {
        ...
        XslTokenParser::make();
        [$root, $attribute] = $this->mockRoot();
        $this->assertNotEquals(XslTokenParser::parse($value, $root, $attribute)->evaluate(), $expected);
    }
    ...
    private function mockRoot(): array
    {
        ...
        return [$mock, new TypedAttribute('color', ColorRgbType::class)];
    }
}
